			...
			if (startSaving){
				...
				for (size_t i = 0; i < line.length(); ++i) {
					...
					if(c=='G'){	
						...
						pictures_a_x.push_back({{lineCounter,static_cast<int>(i)}});
					}

					else if(c=='b'&&(before=='-'|| after=='-')){
						pictures_b_x.push_back({{lineCounter, static_cast<int>(i)}});
					}
					else if(c=='T'){
	...
	for (size_t columnIndex=0; columnIndex < transposedMap.size(); columnIndex++){
		...
		for (int i = 0; i < static_cast<int>(transposedMap[columnIndex].length()); ++i) {
			...
			if (c=='a' && (before == '|' || after == '|')){
				...
				pictures_a_y.push_back({{static_cast<int>(i), static_cast<int>(columnIndex)}});
			}

			if (c=='b' && (before == '|' || after == '|')){
				//std::cout<<"Transposed X coordinate: "<< static_cast<int>(i) << " Transposed Y coordinate: " <<columnIndex<<std::endl;
				pictures_b_y.push_back({{static_cast<int>(i), static_cast<int>(columnIndex)}});
			}
		}
...
Labyrinthe::Labyrinthe (char* filename) : _width(LAB_WIDTH), _height(LAB_HEIGHT)
{
	...
	*/
	...
	for(int i=0; i<static_cast<int>(all_data[4].size()); i++){
		...
		_picts [i]._x2 = all_data[4][i][0][0]; _picts [i]._y2 = (all_data[4][i][0][1]-2);	
		_picts [i]._ntex = 0; 
	}
	...
	for(int j=all_data[4].size(); j<static_cast<int>((all_data[4].size()+all_data[5].size())); j++){
		...
		_picts [j]._x2 = (all_data[5][i][0][0]-2); _picts [j]._y2 = all_data[5][i][0][1];	
		_picts [j]._ntex = 0; 
	}
	...
	for(int j=(all_data[4].size()+all_data[5].size()); j<static_cast<int>((all_data[4].size()+all_data[5].size()+all_data[6].size())); j++){
		...
		_picts [j]._x2 = all_data[6][i][0][0]; _picts [j]._y2 = (all_data[6][i][0][1]-2);	
		char	tmp [128];
		sprintf (tmp, "%s/%s", texture_dir, "voiture.jpg");
	...
	for(int j=all_data[4].size()+all_data[5].size()+all_data[6].size(); j<static_cast<int>((all_data[4].size()+all_data[5].size()+all_data[6].size()+all_data[7].size())); j++){
		...
		_picts [j]._x2 = (all_data[7][i][0][0]-2); _picts [j]._y2 = all_data[7][i][0][1];	
		char	tmp [128];
		sprintf (tmp, "%s/%s", texture_dir, "voiture.jpg");
		_picts [j]._ntex = wall_texture (tmp);	
	}
	...
	for (int g = 1; g < _nguards; g++){
		if (g%2==0){
			_guards[g] = new Gardien (this, "Serpent",200); _guards[g] -> _x = scale*static_cast<float>(all_data[1][g-1][0][0]); _guards[g] -> _y = scale*static_cast<float>(all_data[1][g-1][0][1]);
		}
		else{
			_guards[g] = new Gardien (this, "Samourai",200); _guards[g] -> _x = scale*static_cast<float>(all_data[1][g-1][0][0]); _guards[g] -> _y = scale*static_cast<float>(all_data[1][g-1][0][1]);
		}
